package transliterator4s.error

import scala.util.matching.Regex

sealed abstract class Error(val message: String)

sealed abstract class ParsingFailure(override val message: String) extends Error(message)
...
sealed abstract class TransliterationFailure(override val message: String) extends Error(message)
